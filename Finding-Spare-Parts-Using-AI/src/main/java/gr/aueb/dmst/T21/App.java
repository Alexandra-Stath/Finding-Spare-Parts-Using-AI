...
public class App
{   protected static String[] partsName = new String[253];  //  Ορισμός του πίνακα που περιέχει όλα τα μέλη (ανταλλακτικά ενός αυτοκινήτου που έχουν μοναδικό κωδικό)
    ...
    public String model() throws InputMismatchException {
        ...
        int key1 = 0;  //  Ορίζω το key1 στο οποίο θα αποθηκευτεί η πρώτη επιλογή του χρήστη
        ...
        System.out.printf("Brand:");  // Ζητάω να μου δώσει τη μάρκα του αυτοκινήτου για το οποίο ενδιαφέρεται
        int c = 0; // Αρχηκοποιώ μία μεταβλητή που θα χρησιμοποιήσω για να ελέγξω αν συνέβει κάποιο exception
        do {  // Χρησιμοποιώ μία do-while ώστε να απαιτείται από το χρήστη να ξαναδώσει την τιμή που του ζητήθηκε αν αυτή που έδωσε προηγουμένως ήταν μη επιτρεπόμενη ή προκάλεσε καποιο exception
            if (c == 1) {  //  ελέγχω αν έχει συμβεί κάποιο exception και αν ναι μηδενίζω ξανά την τιμή του c
                c = 0;
            }
            try {  //  χρησιμοποιώ το σχήμα try-catch ώστε να διαχειριστώ το InputMismatchException που μπορεί να προκύψει
                brand = input.nextLine();
            } catch (InputMismatchException e) {
                System.out.printf("ERROR: Inappropriate argument passed into the system, please enter a valid argument.");
                input.nextLine(); // Στο σημείο αυτό το πρόγραμμα οδηγούσε σε ατέρμων βρόγχο και τύπωνε επ άπειρο το παραπάνω μήνυμα στην οθόνη
                c = 1;  // Εάν συνέβει πράγματι η εξαίρεση τότε αποδίδω την τιμή 1 στην μεταβλητή c
            }
        } while(c == 1);  // Εάν η τιμή της c είναι ίση με 1 τότε αυτό σημαίνει ότι έχει συμβεί εξαίρεση η οποία έχει προηγηθεί και πρέπει ο χρήστης να ξαναδώσει την τιμή που του ζητήθηκε
        System.out.printf("\nModel:"); // Το ίδιο ακριβώς μοντέλο χρησιμοποιήθηκε και στην απόδοση τιμής στην μεταβλητή model
        do {
            if (c == 1) {
                c = 0;
            }
        ...
        System.out.printf("\nYear of production:");  //  Το ίδιο ακριβώς μοντέλο χρησιμοποιήθηκε και στην απόδοση τιμής στην μεταβλητή productionYear
         do {
            if (c == 1) {
                c = 0;
            }
            ...
            if (productionYear < 1886) {  //  Η μόνη διαφορά είναι ότι έχω προσθέσει έναν επιπλέον έλεγχο ώστε να συγουρευτώ ότι το έτος παραγωγής είναι μία τιμή
               c = 1;                     //  μεγαλύτερη από το έτος παραγωγής του πρώτου αυτοκινήτου που φτιάχτηκε ποτέ
            }
        } while(c == 1);  
        System.out.printf("\n");
        System.out.printf("Please enter \"1\" or \"2\" to choose an option:\n1) MAIN MENU\n2) SEARCH BAR\n");  //  Δίνω την επιλογή στο χρήστη να επιλέξει αν θέλει να βρεί το κομμάτι
        do {
            ...
            } catch (InputMismatchException e) {  // Ελέγχω αν ο χρήστης έδωσε κάποια μεταβλητή διαφορετικού τύπου από integer
        ...
        } while (c == 1); //  Επαναλαμβάνω την διαδικασία μέχρι ο χρήστης να δώσει μία αποδεκτή τιμή
        String key3;  //  Oρισμός της μεταβλητής key3 στην οποία αποθηκεύεται η 3η επιλογή του χρήστη εάν χριαστεί
        Scanner k = new Scanner(System.in);    
        if (key1 == 1) {  //  Εάν επέλεξε την πρώτη επιλογή του εμφανίζει το πρώτο μενού
            int key2 = 0;  //  Ορίζω την μεταβλητή στην οποία θα αποθηκευτεί η 2η επιλογή του χρήστη
            System.out.printf("Please enter \"1\", \"2\", \"3\", \"4\", \"5\" or \"6\" to select a category:\n" +
            "1) STEERING PARTS\n2) CLUTCH PARTS\n3) ENGINE PARTS\n4) PROPELLER SHAFT\n5) BRAKE PARTS\n6) EXTRA\n");  //  Εμφανίζω τις κατηγορίες στις οποίες χωρίζονται τα ανταλλακτικά
            do {  //  Ελέγχω με αντίστοιχο τρόπο όπως και παραπάνω ότι ο χρήστης έδωσε αποδεκτή τιμή
                try {  //  και δε προκλήθηκε κάποιο exception και αν συνέβησε κάτι τέτοιο ξαναζητάω από το χρήστη να μου δώσει μία αποδεκτή τιμή
                    key2 = input.nextInt();
                    } catch (InputMismatchException e) {
                    System.out.printf("ERROR: Inappropriate argument passed into the system\nplease enter one " + 
                    "of the accepted values as following:\n \"1\", \"2\", \"3\", \"4\", \"5\" or \"6\"\n");
            ...
            if (key2 == 1) {
                System.out.println("1.1 UPPER CONTROL ARM\n1.2 UPPER BALL JOINT\n1.3 COIL SPRING\n" +  //  Εμφανίζω τις επιλογές ανταλλακτικών-μελών που αφορούν την 1η κατηγορία
                ...
                System.out.println("3.1 FUEL PUMP\n3.2 CYLINDER HEAD COVER\n3.3 GASKET\n" +  //  //  Εμφανίζω τις επιλογές ανταλλακτικών-μελών που αφορούν την 3η κατηγορία
                ...
                System.out.println("6.1 THERMOSTAT\n6.2 RESERVOIR TANK\n6.3 FUEL TANK\n6.4 FUEL TANK PRESSURE SENSOR\n6.5 OXYGEN SENSOR\n" +  //  //  Εμφανίζω τις επιλογές ανταλλακτικών-μελών που αφορούν την 6η κατηγορία
            ...
            do {
                ...
                do {
                    System.out.println("Give the name of the part in capital letters");  //  Εμφανίζω τα κατάλληλα μηνύματα και οδηγίες
                ...
                System.out.printf("\n");
                input.close();  // Κλείνω το Scanner input
                k.close();  // Κλείνω το Scanner k
            }
            String prompt = "Give me details about " + key3 + ", " + brand + " " + model + " " + productionYear + ".";  // Συντάσσω το ερώτημα που πρέπει να απευθύνω στο AI
            return prompt;
    }
}
